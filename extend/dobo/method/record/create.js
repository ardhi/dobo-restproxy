...
async function recordCreate ({ schema, body, options = {} } = {}) {
  ...
  const mod = await importModule(`${prefix}/lib/${adapter.type}/record-create.js`)
  if (!mod) return unsupported.call(this)
  let { url, opts, ext } = await prepFetch.call(this, schema, 'create', undefined, body)
  let resp
  if (isFunction(mod)) ({ url, opts, ext, resp } = await mod.call(this.app[adapter.ns], { url, opts, ext, schema, body, options }))
